...
base_db = r"C:\Users\Admin\.gemini\antigravity\scratch\krishidrishti-edge\backend\app\db"
...
with open(os.path.join(base_db, "database.py"), "w", encoding="utf-8") as f:
...
with open(os.path.join(base_db, "models.py"), "w", encoding="utf-8") as f:
    f.write(models_code)

# 3. __init__.py
init_code = """from app.db.database import Base, engine, SessionLocal, get_db, init_db
...
with open(os.path.join(base_db, "__init__.py"), "w", encoding="utf-8") as f:
    f.write(init_code)

print("Database and Models created successfully.")
